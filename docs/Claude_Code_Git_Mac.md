...
- **Finder:** Open **Applications** > **Utilities** > **Terminal**
- **Launchpad:** Click **Launchpad** in the Dock, search for `Terminal`

You'll see a command prompt ending with `$` or `%`.

## Step 2: Install Git

Mac often has Git pre-installed. Let's check:
...
**If you see a version number** (like `git version 2.39.0`), Git is already installed—skip to Step 3.

**If you see "command not found"** or a popup asking to install developer tools:
- Click **Install** in the popup, or run:
  ```
...
You should see `/Users/YOUR_USERNAME/Documents`.
...
- In Claude Code, type:
  ...
  ```
- Refresh your browser tab (or press `Command (⌘) + R`)
...
- In Claude Code, type:
  ...
  ```
- Refresh your browser tab (press `Command (⌘) + R`)
...
- In the browser that shows the app, right click and select **View Page Source** (or press `Option (⌥) + Command (⌘) + U`)
