...
Claude Code es un potente asistente de codificación con IA, pero los usuarios de Windows enfrentan un desafío: muchas herramientas de desarrollo funcionan mejor en Linux. ¿La solución? WSL (Windows Subsystem for Linux) le permite ejecutar un entorno Linux completo dentro de Windows, como tener dos computadoras en una. Esta guía le llevará a través de la configuración completa para que pueda comenzar a programar con Claude en aproximadamente 20 minutos.

## Conceptos Clave

- **WSL (Windows Subsystem for Linux)** - Una característica de Windows que ejecuta un sistema Linux real junto con Windows
- **Ubuntu** - Una distribución de Linux popular y amigable para principiantes que instalaremos a través de WSL
- **PowerShell** - La herramienta de línea de comandos integrada de Windows, utilizada aquí para instalar WSL
- **Node.js** - Un entorno de ejecución de JavaScript que Claude Code requiere para funcionar
- **Terminal** - Una interfaz basada en texto para ejecutar comandos (como la línea de comandos de Ubuntu)
...
- Durante el inicio, presione la tecla del BIOS (generalmente **F2**, **F10**, **Del** o **Esc** - depende del fabricante de su computadora)
...
- Guarde y salga del BIOS (generalmente **F10**)
...
- En los resultados de búsqueda, verá **Windows PowerShell**
- **Haga clic derecho** en **Windows PowerShell**
- Haga clic en **Ejecutar como administrador** en el menú
...
- Observe los resultados:
   ...
   - **Si ve un mensaje de error** o "Windows Subsystem for Linux has no installed distributions": Continúe con la instalación a continuación.
...
**Por qué es necesario reiniciar:** El reinicio permite que Windows habilite las características de WSL y Virtual Machine Platform que acaban de instalarse.

**Nota:** Si obtiene un error diciendo que el comando no se reconoce, su versión de Windows puede ser demasiado antigua. Asegúrese de tener Windows 10 versión 2004 o superior, o Windows 11. Ejecute Windows Update para obtener la última versión.

## Paso 4: Configurar Ubuntu (Solo la Primera Vez)

Después de que su computadora se reinicie, una ventana de terminal con "Ubuntu" en el título debería abrirse automáticamente dentro de 2-5 minutos.
...
- Escriba un nombre de usuario (use solo letras minúsculas y números, sin espacios)
   - Ejemplo:  `john`
- Verá: `New password:`
...
- En la terminal de Ubuntu, copie y pegue estos comandos:
   ...
   nvm install 24
   ```
   Esto primero descarga el instalador de nvm, lo instala y luego lo usa para instalar node.js v 24.0
...
- Puede ver algunas advertencias en texto amarillo o rojo - esto es usualmente normal
...
- En la terminal de Ubuntu, escriba:
   ...
   claude
   ```
- Claude intenta abrir un navegador. Si no puede abrirlo automáticamente, mantenga presionado **Ctrl** y haga clic en la URL larga para abrirla en un navegador. Alternativamente, copie la URL y péguela en un navegador externo.
- Inicie sesión en su cuenta de Claude.ai (esto puede suceder automáticamente si está usando Chrome)
...
- Primero, obtenga su clave API del [Anthropic Console](https://console.anthropic.com/)
- En la terminal de Ubuntu, escriba:
   ```
...
Antes de iniciar Claude Code, en la ventana de terminal de Ubuntu, pegue este código para definir las variables de entorno:
```
...
**Nota:** Reemplace `xxxx-eastus2` con el nombre de su Foundry Resource (no use la URL base completa). Reemplace `your_api_key` con su clave API completa de su portal de Azure.
...
- Si tiene una carpeta de Windows llamada `test_claude` que contiene los archivos de un proyecto, puede acceder a ella:
   ```
...
- Luego inicie Claude:
   ...
   claude
   ```
- Comience pidiendo a Claude que le explique el código base.
- Puede pedir a Claude que haga cambios.
...
- Si está deshabilitada, necesita habilitarla en el BIOS de su computadora (vea el Paso 1 para instrucciones)
