...
- In the Ubuntu terminal, type this command (replace `YourUsername` with your actual Windows username):
  ```
  cd /mnt/c/Users/YourUsername/Documents/iris_analysis
  ```
...
- Highlight the new code and click **Run**
- A scatter plot appears in the **Plots** pane (bottom-right)
- If you don't have ggplot2 package installed, you will have to install it

## Step 10: Ask Claude for PCA Plot
...
## Step 11: Run the PCA Analysis

- Switch to RStudio
- Highlight the new code and click **Run**
- The plot should be changed
...
- Type this request:
  ```
  Review the entire script for correctness. Add comments when neccessary. 
  ```
...
- **Plots not appearing** - Make sure required packages (ggplot2, corrplot, etc.) are installed. Run `install.packages(c("ggplot2", "corrplot"))` in RStudio Console if needed.
- **Error: "cannot change working directory"** - Your Windows path contains spaces. In Step 6, wrap the path in quotes: `cd "/mnt/c/Users/Your Name/Documents/iris_analysis"`

## Next Steps

- Try asking Claude to add statistical tests (t-tests, ANOVA) to your analysis
- Ask Claude to get a **Python version** of this code and prepare a Quardo document
