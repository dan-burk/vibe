...
你想编写 R 代码，但觉得 RStudio 太重，或者你更喜欢 VS Code 的灵活性。可以把 VS Code 想象成瑞士军刀——它可以在一个轻量级编辑器中处理 R、Python 和许多其他语言。本教程将向你展示如何在 VS Code 中设置 R，并使用代码补全、交互式绘图甚至 Shiny 应用等智能功能。
...
你需要 R 4.0 或更高版本以获得最佳兼容性。

- **Windows**：
  - 从 [CRAN Windows](https://cran.r-project.org/bin/windows/base/) 下载最新的 R 并运行安装程序
  - 安装过程中，**记下安装路径**（例如：`C:\Program Files\R\R-4.5.3`）
  - 如果你有旧版本，请先通过"设置 > 应用"卸载它

- **macOS**：
  - 从 [CRAN macOS](https://cran.r-project.org/bin/macosx/) 下载并运行 .pkg 安装程序
  - 记下你使用的是 Intel（`/Library/Frameworks/R.framework/Resources/bin/R`）还是使用 Homebrew 的 Apple Silicon（`/opt/homebrew/bin/R`）
  - 如果是更新，新版本将替换旧版本

- **Linux**：
  - 使用你的包管理器（例如，在 Ubuntu 上使用 `sudo apt install r-base`）
  - 或遵循 [CRAN Linux](https://cran.r-project.org/bin/linux/) 说明

你将在步骤 3 中配置 VS Code 以找到 R，所以如果 `R --version` 在终端中还不起作用，不必担心。
...
在配置 VS Code 之前，你需要知道 R 安装在系统的什么位置。

### Windows 用户

1. 打开**文件资源管理器**
2. 导航至 `C:\Program Files\R\`
3. 你将看到类似 `R-4.5.3` 的文件夹（你的版本号可能不同）
4. 打开该文件夹 → 打开 `bin` 文件夹
5. 你会在那里看到 `R.exe`
6. 完整路径为：`C:\Program Files\R\R-4.5.3\bin\R.exe`

**记下或记住你的版本号**（例如，`R-4.5.3`）——你在步骤 4 中需要用到它。
...
- 你应该会看到该设置出现。点击设置下方的**在 settings.json 中编辑**
- settings.json 文件将打开
- 在花括号 `{}` 内为你的平台添加相应的配置
...
**为什么要用两个反斜杠（`\\`）？** 在 JSON 文件中，反斜杠 `\` 是一个特殊字符（称为转义字符）。要在实际文件路径中表示单个反斜杠，你必须输入两个反斜杠 `\\`。因此，`C:\Program Files` 在 settings.json 中变成 `C:\\Program Files`。
...
或者，如果你在 Apple Silicon 上通过 Homebrew 安装 R：
...
仅在常规终端中输入 `R` 是不起作用的，除非 R 在你的系统 PATH 中。R 扩展使用你的 `r.rpath` 设置来找到 R。

## 步骤 6：创建你的 R 项目

- 在你的计算机上创建一个新文件夹（例如，`my-r-project`）
- 在 VS Code 中，点击**文件 > 打开文件夹**并选择你的新文件夹
- 点击顶部菜单中的**文件 > 新建文件**
...
- 如果你没有打开终端，第一次 `Ctrl+Enter` / `Cmd+Enter` 会打开 R 终端。第二次会运行代码。
- 观察输出出现在终端中，图形将在单独的窗口中打开
- 你也可以选择所有行并以相同方式运行它们。

## 步骤 9：创建一个简单的 Shiny 应用

- 在你的项目文件夹中创建一个新文件：**文件 > 新建文件**
- 将其保存为 `app.R`
- 输入以下代码：
...
- **Git 集成**：使用 VS Code 的内置 Git 功能对你的 R 项目进行版本控制
...
- **VS Code 终端中"无法识别 R"**：VS Code 找不到 R。检查你的 settings.json（步骤 4）——验证 R 路径是否正确并指向 `R.exe`（Windows）或 R 二进制文件（Mac/Linux）。示例路径：
...
- **settings 中的 R 版本不匹配**：如果你更新了 R 但 VS Code 仍在使用旧版本，请更新 settings.json 中的 `r.rpath` 配置中的版本号以匹配新的 R 安装（参见步骤 3 以找到你当前的安装路径）。

- **代码补全不起作用**：确保 languageserver 已成功安装（步骤 5）。安装后重启 VS Code。打开 R 文件后等待 10-20 秒以便语言服务器初始化。检查输出面板（查看 > 输出 > R Language Server）中的错误。

- **图形未显示**：图形将使用 R 的默认图形设备在单独的窗口中打开。确保你有可用的窗口管理器。在某些系统上，你可能需要安装 X11（macOS）或确保启用了图形支持。

- **Shiny 应用无法运行**：确保已安装 Shiny 扩展（步骤 2）、已安装 shiny 包（步骤 5）且你的文件名为 `app.R`。检查终端中的错误消息。仅当 VS Code 将文件识别为 Shiny 应用时，运行按钮才会出现。
