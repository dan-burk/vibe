...
- 查找右下角的通知：**Folder contains a Dev Container configuration file**
...
该项目包含一个演示 Shiny 应用程序，用于创建交互式直方图。
...
让我们修改应用程序以了解开发是如何工作的。

- 保持应用程序运行
- 在 VS Code 中，编辑 `R/app.R`
...
- `rocker/shiny-verse:latest` - 带 Shiny 和 tidyverse 的 R（我们正在使用的）

更改基础镜像后，重新构建容器以应用更改。

## 步骤 11：在 Docker 镜像中安装 R 包（可选）

通过 R 控制台（`install.packages()`）安装的包是临时的，在重新构建容器时会消失。要使包永久存在，请将它们添加到 Dockerfile 中。
...
- **创建新的 R 脚本** - 在 `R/` 文件夹中创建新的 `.R` 文件，使用内置数据集（如 `mtcars` 或 `iris`）编写数据分析代码
- **安装 R 包** - 通过编辑 Dockerfile 并重新构建容器来添加所需的包
- **探索 tidyverse** - 尝试使用 `dplyr` 进行数据操作，使用 `ggplot2` 和示例数据集进行可视化
