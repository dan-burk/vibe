...
你正在使用 AI 辅助编程。它会对你的文件进行修改。有时这些修改完美无缺，有时却不尽如人意。**版本控制就像是整个项目的撤销按钮。**每次保存快照（称为"提交"）时，你就创建了一个还原点。你随时可以回退。

当你将 [GitHub Desktop](https://desktop.github.com) 与 [Claude Code](https://claude.com/claude-code) 结合使用时，你就能获得 AI 加速开发与专业版本控制的完美组合。

## 关键概念

**Git** 在你的计算机上跟踪文件的每一次更改。

**GitHub** 将你的代码存储在云端作为备份。

**GitHub Desktop** 让 Git 变得可视化 - 你只需点击按钮而无需输入命令。

**Claude Code** 是一个 AI 编程助手，可以为你编写代码、修复错误并创建提交。
...
有时 AI 会让我们走上错误的道路，我们需要从上次提交（保存点）重新开始。
