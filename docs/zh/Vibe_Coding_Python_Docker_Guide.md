...
你曾经通过自己逐行输入来编写 Python 代码。但如果你可以用简单的英语描述你想要的功能，然后看着代码自动生成呢？Vibe 编程就像与你的计算机对话——你描述结果，Claude Code 构建它，你测试并完善。这不是魔法，这是一种新的工作方式，你引导愿景，AI 处理实现。本教程将向你展示如何仅使用自然语言请求来分析经典的鸢尾花数据集。

## 核心概念

- **[Vibe Coding](https://www.ibm.com/think/topics/vibe-coding)** - 通过用自然语言描述你想要的功能来编程，然后根据结果进行迭代，而不是逐行编写代码
...
- 如果你希望公开，取消勾选 **Keep this code private**（可选）
- 点击 **Publish Repository**

你现在拥有一个本地 Git 仓库和 GitHub 上的备份。

## 步骤 2：复制 Docker 配置

你需要从 vibe 项目复制 `.devcontainer` 文件夹来设置你的 Docker 环境。

- 打开 File Explorer（Windows）或 Finder（Mac）
- 导航到你的 vibe 项目文件夹（例如，`Documents/vibe`）
- 找到 `.devcontainer` 文件夹
- 复制整个文件夹（它包含 `Dockerfile` 和 `devcontainer.json`）
- 导航到你的新 `iris-analysis` 文件夹
- 将 `.devcontainer` 文件夹粘贴到那里

你的 `iris-analysis` 文件夹现在应该包含：
- `.devcontainer/`（你刚刚复制的文件夹）
...
- 如果你没有看到通知，点击左下角的绿色图标并选择 **Reopen in Container**
...
Claude Code 现在正在运行并准备好接收你的请求。

## 步骤 5：第一次 Vibe - 加载数据

现在有趣的部分开始了。无需查找文档，只需描述你想要的功能。
...
要求 Claude 使用 Git 提交。或者你自己从 GitHub Desktop 执行此操作。
```
...
- **无法推送到 GitHub** - 确保你已登录 GitHub Desktop 并发布了仓库（步骤 1）。检查你的互联网连接。
...
魔法不在于任何单一工具——而在于 vibe 编程如何让你描述你想要的内容并快速迭代。你在 20 分钟内从空项目到具有多个可视化的完整数据分析，而无需手动编写一行代码。
