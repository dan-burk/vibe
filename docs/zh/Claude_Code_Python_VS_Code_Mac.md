...
- 在搜索框中，输入 `Claude Code`
- 找到 Anthropic 的 **Claude Code** 并点击 **Install**
...
- 点击 **Select a language** 并输入 `Python`，然后选择 **Python**
- 通过点击 **File > Save** 保存文件，命名为 `iris_analysis.py`
- 输入以下代码：
...
- Claude 会显示更新后的代码更改
- 点击 **Accept**
- 再次运行更新后的代码：在 Terminal 中输入 `python iris_analysis.py`
- 图表现在会显示为每个物种使用不同的标记形状，没有标题，并且使用 whitegrid 样式
- 关闭图表窗口继续
...
- 使用 Claude 调试错误："This code gives Error X, can you fix it?"
...
- 获取不熟悉函数的帮助："Explain what pandas.groupby does"
