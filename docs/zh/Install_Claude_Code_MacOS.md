...
Claude Code 是一个运行在终端中的 AI 助手，可以帮助你编写、调试和理解代码。可以把它想象成一个随时待命的知识丰富的编程伙伴。无论你是完全的初学者还是经验丰富的开发者，Claude Code 都能加速你的工作流程并帮助你学习。

本指南将逐步引导你完成安装，并为初学者提供详细的说明。
...
- **Terminal**：Mac 内置的应用程序，你可以在其中输入命令而不是点击按钮。这是你与 Claude Code 交互的方式。
- **Node.js**：Claude Code 运行所需的软件。可以把它想象成驱动 Claude Code 的引擎。
- **Claude Code**：在 Terminal 中运行的 AI 编程助手。它可以回答问题、编写代码并帮助你理解现有项目。
...
- 文件将下载到你的 Downloads 文件夹（通常需要 30-60 秒）
...
- 在 Terminal 中输入：
  ...
  ```
  将 `your-api-key-here` 替换为你实际的 API 密钥
- 要使其永久生效（这样你就不必每次都设置），请将其添加到 shell 配置文件中：
  ```
...
**注意：** 如果你使用的是较旧的 Mac，使用的是 bash 而不是 zsh，请在上述命令中将 `~/.zshrc` 替换为 `~/.bash_profile`。
...
**注意：** 将 `xxxx-eastus2` 替换为你的 Foundry Resource 名称（不要使用完整的基础 URL）。将 `your_api_key` 替换为你从 Azure 门户获取的完整 API 密钥。
...
- 如果你的 Mac 文件夹中有项目，可以导航到它：
  ```
  cd ~/Documents/test_claude
  ```
  将 `test_claude` 替换为你实际的项目文件夹名称
